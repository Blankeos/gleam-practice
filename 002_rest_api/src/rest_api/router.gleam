...
fn home_page(req: Request) -> Response {
  ...
  let html =
    string_builder.from_string(
      string.concat([
        "<h2>Hello, Carlo! API Documentation</h2>", "<p>/comments</p>",
        "<p>/comments/:id</p>",
      ]),
    )
...
fn show_comment(req: Request, id: String) -> Response {
  ...
  let object =
    json.object([
      #("name", json.string("carlo")),
      #("commentId", json.string(id)),
      #("is-cool", json.bool(True)),
      #("saved", json.bool(True)),
    ])

  wisp.ok()
  |> wisp.json_body(json.to_string_builder(object))
  // Alternatively:
